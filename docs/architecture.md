...
| 2026-05-28 | v1.25, Module 43 address bar + Phase 8 product decisions | **Module 43 (Address bar) done.** Key decisions locked: (1) **`BadgeEvent::Reset` ownership moved to Module 21** — the address bar does NOT reset the tracker counter on `NavigatePressed`; the network layer fires Reset when a new page begins loading. This preserves seeded demo data in stub builds and is architecturally correct (address bar does not know when the page actually loads). (2) **Standard→Strict conversion is in-place** (product decision overrides §3.1 "mode locked at creation"): `ConvertToStrictClicked` immediately sets `mode = Strict` on the current tab; no new tab is opened. Strict→Standard remains forbidden (§3.6). The 600 ms morph animation path (`TransitioningMode` / `MorphTick`) is still in shell.rs but not triggered by the button — it activates when a timer subscription is wired post-Phase 8. (3) **`ChipState` enum** governs Convert-to-Strict chip visibility: `FreshTab` (new tab, no URL — chip shown until Enter or dismiss), `ExternalUrl` (URL opened from outside — chip shown for 30 s via `Task::perform` timer then auto-dismissed), `Hidden` (permanent — user pressed Enter, timer fired, or user clicked ×). `NavigatedExternally(url)` message carries external URLs; fired by Module 80 orchestrator (Phase 11) via `ChromeCommand::ExternalNavigation`. (4) **`pb-ui::design` renamed from `pb-ui::tokens`** — `src/tokens.rs` deleted, `src/design.rs` created, `OUT_DIR/tokens.rs` → `OUT_DIR/design.rs` in build.rs; all consumers updated. (5) **SVG shield icons**: outlined shield (`assets/shield-outline.svg`) on the chip (pre-Strict), filled shield (`assets/shield.svg`) in the Strict badge (post-conversion). Iced `svg` feature added to workspace Cargo.toml. (6) **`show_input = focused \|\| !has_url`** eliminates extra click on fresh tab; typing auto-transitions `bar_state` from `Rest` → `Focused` so Enter submits. **No new L-invariants.** |
